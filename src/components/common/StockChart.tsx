    ...
    <Box sx={{ width: '100%' }}>
      ...
      {(calculatedMetrics.quantity > 0 || transactions.length > 0) && (
        <Box sx={{ ml: '40px', mt: 2, mb: 4, display: 'grid', gridTemplateColumns: 'auto auto auto auto', columnGap: '120px', rowGap: '24px' }}>
            <Box><Typography variant="body2" sx={{ color: '#a0a0a0' }}>Owned</Typography><Typography variant="h6" fontWeight="bold">{calculatedMetrics.quantity.toFixed(2)}</Typography></Box>
            <Box><Typography variant="body2" sx={{ color: '#a0a0a0' }}>Sum of Cost</Typography><Typography variant="h6" fontWeight="bold">${calculatedMetrics.totalCost.toFixed(2)}</Typography></Box>
            <Box><Typography variant="body2" sx={{ color: '#a0a0a0' }}>Total Fees</Typography><Typography variant="h6" fontWeight="bold">${calculatedMetrics.totalCommission.toFixed(2)}</Typography></Box>
            <Box><Typography variant="body2" sx={{ color: '#a0a0a0' }}>{calculatedMetrics.totalProfit >= 0 ? 'Total Gain' : 'Total Loss'}</Typography><Typography variant="h6" fontWeight="bold" sx={{ color: calculatedMetrics.totalProfit >= 0 ? '#00C805' : '#FF3B30' }}>{calculatedMetrics.totalProfit >= 0 ? '+' : ''}{calculatedMetrics.totalProfit.toFixed(2)}</Typography></Box>
            ...
            <Box><Typography variant="body2" sx={{ color: '#a0a0a0' }}>Unrealized Profit</Typography><Typography variant="h6" fontWeight="bold" sx={{ color: calculatedMetrics.unrealizedProfit >= 0 ? '#00C805' : '#FF3B30' }}>{calculatedMetrics.unrealizedProfit >= 0 ? '+' : ''}{calculatedMetrics.unrealizedProfit.toFixed(2)}</Typography></Box>
            <Box><Typography variant="body2" sx={{ color: '#a0a0a0' }}>Realized Profit</Typography><Typography variant="h6" fontWeight="bold" sx={{ color: calculatedMetrics.realizedProfit >= 0 ? '#00C805' : '#FF3B30' }}>{calculatedMetrics.realizedProfit >= 0 ? '+' : ''}{calculatedMetrics.realizedProfit.toFixed(2)}</Typography></Box>
        </Box>
      )}
                ...
                {transactions.length === 0 ? (
                  ...
                  transactions.map((row, index) => (
                    <TableRow key={index} sx={{ '&:last-child td, &:last-child th': { border: 0 }, transition: 'background-color 0.2s', '&:hover': { bgcolor: 'rgba(255,255,255,0.04)' } }}>
                      ...
                      <TableCell align="right" sx={{ color: 'white', borderBottom: '1px solid rgba(255,255,255,0.05)', py: 2 }}><Typography variant="body2" fontWeight="bold">{row.quantity}</Typography></TableCell>
                      <TableCell align="right" sx={{ color: 'white', borderBottom: '1px solid rgba(255,255,255,0.05)', py: 2 }}><Typography variant="body2">${row.price.toFixed(2)}</Typography></TableCell>
                      <TableCell align="right" sx={{ color: 'white', borderBottom: '1px solid rgba(255,255,255,0.05)', py: 2 }}><Typography variant="body2" fontWeight="bold">${row.totalCost.toFixed(2)}</Typography></TableCell>
